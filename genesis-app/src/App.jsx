...
function PlanejamentoView({ workPackages, updWp, materials, setReportFn, setExportXlsxFn, allPortCallDates, portCallLabel, addWpOnDate,
  ...
  /* ========================================================
     QUADRO KANBAN — visão tipo Trello dos itens mapeados + docagem, organizados por Status
     (arrastar o cartão entre as colunas muda o status). Preencher a Data de Execução no
     cartão já dispara a graduação automática pro Port Call (mesmo efeito de sempre, só que
     agora dá pra fazer isso direto no cartão, sem precisar abrir a tabela). */
  const [kanbanBusca, setKanbanBusca] = useState("");
  const [draggedCard, setDraggedCard] = useState(null);
  const [dragOverStatus, setDragOverStatus] = useState(null);
  /* o quadro mostra 3 origens ao mesmo tempo: itens de Mapeados e de Docagem (sempre aparecem,
     a não ser que tenham sido "removidos só do quadro" — hiddenFromKanban) e cartões criados
     manualmente direto aqui (guardados à parte, em kanbanCards, sem relação com nenhuma outra aba) */
  const kanbanBoardCards = useMemo(() => {
    const norm = (s) => (s || "").toString().toLowerCase();
    const fromPlan = planningItems.filter((p) => !p.hiddenFromKanban).map((p) => ({
      key: `plan-${p.id}`, id: p.id, source: "plan",
      nome: p.nome || "Mapeamento sem nome", subtitulo: p.departamento || p.empresa || "",
      status: p.status || "A Executar", dataExecucao: p.dataExecucao || "",
      linkedServiceId: p.linkedServiceId, impacto: p.impacto || "Baixo", planoAcao: p.planoAcao || "",
    }));
    const fromDoc = docagemItems.filter((d) => !d.hiddenFromKanban).map((d) => ({
      key: `doc-${d.id}`, id: d.id, source: "doc",
      nome: d.nome || "Item de docagem", subtitulo: d.localizacao || "",
      status: d.status || "A Executar", dataExecucao: d.previsaoExecucao || "",
      linkedServiceId: d.linkedServiceId, impacto: null, planoAcao: d.planoAcao || "",
    }));
    const fromManual = kanbanCards.map((k) => ({
      key: `manual-${k.id}`, id: k.id, source: "manual",
    ...
    const all = [...fromPlan, ...fromDoc, ...fromManual];
  ...
  }, [planningItems, docagemItems, kanbanCards, kanbanBusca]);
  ...
  const setKanbanCardStatus = (card, novoStatus) => {
    if (card.source === "plan") {
      const idx = planningItems.findIndex((p) => p.id === card.id);
      if (idx >= 0) updPlan(idx, "status", novoStatus);
    } else if (card.source === "doc") {
      const idx = docagemItems.findIndex((d) => d.id === card.id);
      if (idx >= 0) updDocagem(idx, "status", novoStatus);
    } else {
  ...
  const setKanbanCardData = (card, novaData) => {
    if (card.source === "plan") {
      const idx = planningItems.findIndex((p) => p.id === card.id);
      if (idx >= 0) updPlan(idx, "dataExecucao", novaData);
    } else if (card.source === "doc") {
      const idx = docagemItems.findIndex((d) => d.id === card.id);
      if (idx >= 0) updDocagem(idx, "previsaoExecucao", novaData);
    } else {
      const idx = kanbanCards.findIndex((k) => k.id === card.id);
      if (idx >= 0) updKanbanCard(idx, "dataExecucao", novaData);
    }
  };
  /* "apagar" um cartão do quadro: se ele veio de Mapeados/Docagem, só marca como escondido do
     Kanban (hiddenFromKanban) — o item continua existindo normalmente na aba de origem. Se o
     cartão foi criado manualmente aqui, aí sim é removido de vez (não existe em outra aba). */
  const removeKanbanCard = (card) => {
    if (card.source === "plan") {
      const idx = planningItems.findIndex((p) => p.id === card.id);
      if (idx >= 0) updPlan(idx, "hiddenFromKanban", true);
    } else if (card.source === "doc") {
      const idx = docagemItems.findIndex((d) => d.id === card.id);
      if (idx >= 0) updDocagem(idx, "hiddenFromKanban", true);
    } else {
      const idx = kanbanCards.findIndex((k) => k.id === card.id);
                    ...
                    <div key={card.key}
                      ...
                      <div onClick={(e) => e.stopPropagation()} className="g-flex" style={{ justifyContent: "space-between", alignItems: "flex-start", gap: 4, marginBottom: 4 }}>
                        {card.source === "manual" ? (
                          ...
                          <div style={{ fontSize: 12, fontWeight: 600 }}>{card.nome}</div>
                        )}
                        <button
                          title={card.source === "manual" ? "Excluir cartão" : "Remover só deste quadro (o item continua na aba de origem)"}
                      ...
                      <div className="g-flex" style={{ gap: 5, flexWrap: "wrap", marginBottom: 5 }}>
                        <span className="g-pill" style={{ background: "var(--panel)", fontSize: 10 }}>
                          {card.source === "plan" ? "Mapeamento" : card.source === "doc" ? "Docagem" : "Manual"}
                        </span>
                      ...
                      {card.source === "manual" ? (
                        ...
                        card.subtitulo && <div style={{ fontSize: 10.5, color: "var(--text-faint)", marginBottom: 5 }}>{card.subtitulo}</div>
                      )}
